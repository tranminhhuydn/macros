...
export default {
  ...
  connectData (){
    ...
    for (var i = 0; i < dataRoot.length; i++) {
      ...
      if(_name.indexOf(".json")!=-1){
        ...
        this.listDB[keyName]=  require("../data/"+_name);
      }
    }
  ...
  loadLibScript(){
    ...
    dataDir = []
    ,result = ''
    ,scriptDir = this.path.parse(__dirname);
    scriptDir = path.join(scriptDir.dir, 'libscript')
    dataDir = fs.readdirSync(scriptDir)

    for (var i = 0; i < dataDir.length; i++) {
      var pathFile =path.join(scriptDir,dataDir[i])
      //fs.readFileSync( path.join(dirTranslate,_name))
      //console.log(pathFile);
      result += fs.readFileSync(pathFile, 'utf8')+"\n"
      //eval()
    }
    return result;
  },
